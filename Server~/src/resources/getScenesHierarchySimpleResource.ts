 ...
 * This resource provides a truncated view of the Unity scene hierarchy
 *
...
export function registerGetHierarchySimpleResource(server: McpServer, mcpUnity: McpUnity, logger: Logger) {
  ...
  server.resource(
    ...
    resourceUri,
    {
      description: 'Retrieve a vastly truncated hierarchy of all GameObjects in the Unity loaded scenes',
...
/**
 * Handles requests for truncated hierarchy information from Unity
 *
 * @param mcpUnity The McpUnity instance to communicate with Unity
 * @returns A promise that resolves to the truncated hierarchy data
...
async function resourceHandler(mcpUnity: McpUnity): Promise<ReadResourceResult> {
  ...
  if (!response.success) {
    throw new McpUnityError(
      ...
      response.message || 'Failed to fetch truncated hierarchy from Unity'
    );
  }
